...
export default class Bitrix24Page extends React.Component {
  ...
  componentDidMount() {
    ...
    // Раскладка целиком в CSS (globals.css → «adaptive design tokens»).
    // В JS остаётся только поведение: закрыть меню на десктопной ширине.
  ...
  renderVals() {
    const { mobileMenuOpen, openFaq } = this.state;

    const vals = {
      // Адаптивность приходит из CSS-переменных — значения одинаковы при SSR
      // и на клиенте, поэтому гидратация не ломает вёрстку на телефоне.
      ...R,
      contactColumns: R.cols2,
      kanbanHeight: 'var(--platform-slot-h)',
...
<div style={s(`--bg:#FAFAF8; --paper:#FFFFFF; --ink:#14171C; --ink-soft:#535C69; --ink-faint:#8A8F99; --line:#E7E6E2; --blue:#155EEF; --violet:#12A5E0; --grad:linear-gradient(135deg, var(--blue), var(--violet)); font-family:var(--font-inter),sans-serif; background:var(--bg); color:var(--ink); min-height:100vh; position:relative; isolation:isolate; overflow-x:clip;`)}>
  ...
  <header ref={this.headerRef} className="site-header" style={s(`position:sticky; top:14px; z-index:50; margin:14px ${v.padX} 0; display:flex; align-items:center; justify-content:space-between; padding:12px 14px 12px 20px; background:linear-gradient(135deg, rgba(255,255,255,0.62), rgba(255,255,255,0.34)); backdrop-filter:blur(24px) saturate(180%); -webkit-backdrop-filter:blur(24px) saturate(180%); border:1px solid rgba(255,255,255,0.7); border-radius:22px; box-shadow:0 16px 40px -16px rgba(20,23,28,0.32), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
    ...
    <div style={s(`display:${v.navDisplay}; align-items:center; gap:24px;`)}>
      <a className="b24fx0" href="#steps" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>Этапы</a>
      <a className="b24fx1" href="#integrations" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>Интеграции</a>
      <a className="b24fx2" href="#migration" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>Миграция</a>
      <a className="b24fx3" href="#sla" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>Поддержка</a>
      <a className="b24fx4" href="#faq" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>FAQ</a>
      <a href="/bitrix24" aria-current="page" style={s(`font-size:var(--t15); font-weight:600; color:var(--blue); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--blue),var(--blue)); background-repeat:no-repeat; background-position:0 100%; background-size:100% 1.5px; padding-bottom:3px;`)}>Битрикс24</a>
      <a className="b24fx5" href="/ai" style={s(`font-size:var(--t15); font-weight:500; color:var(--ink-soft); text-decoration:none; white-space:nowrap; background-image:linear-gradient(var(--ink),var(--ink)); background-repeat:no-repeat; background-position:0 100%; background-size:0% 1.5px; padding-bottom:3px; transition:background-size 0.3s cubic-bezier(.16,1,.3,1), color 0.2s ease, transform 0.25s cubic-bezier(.16,1,.3,1);`)}>ИИ-решения</a>
    ...
    <div style={s(`position:relative; z-index:1; display:grid; grid-template-columns:${v.heroCols}; gap:${v.heroGap}; align-items:center;`)}>
      <div>
        ...
        <div style={s(`display:flex; align-items:center; gap:22px; margin-top:34px; flex-wrap:wrap; ${v.entranceCta}`)}>
          ...
          <a className="b24fx9" href="#steps" style={s(`font-size:var(--t16); font-weight:600; color:var(--ink); display:inline-flex; align-items:center; gap:8px; padding:16px 2px; text-decoration:none; border-bottom:1px solid transparent; transition:border-color 0.25s ease;`)}>Этапы внедрения ↓</a>
        </div>

        <div style={s(`display:flex; gap:24px; flex-wrap:wrap; margin-top:38px; ${v.entranceCta}`)}>
          <div style={s(`display:flex; align-items:center; gap:9px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:var(--blue); flex-shrink:0;`)}></span><span style={s(`font-size:var(--t14); color:var(--ink-soft);`)}>Запуск от 2 недель</span></div>
          <div style={s(`display:flex; align-items:center; gap:9px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:var(--violet); flex-shrink:0;`)}></span><span style={s(`font-size:var(--t14); color:var(--ink-soft);`)}>Договор и SLA</span></div>
          <div style={s(`display:flex; align-items:center; gap:9px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:#1F8A5B; flex-shrink:0;`)}></span><span style={s(`font-size:var(--t14); color:var(--ink-soft);`)}>Обучение включено</span></div>
        </div>
      </div>
  ...
  <section id="scope" data-screen-label="Что настраиваем" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal style={s(`max-width:calc(720px * var(--t-scale));`)}>
      <div style={s(`font-family:monospace; font-size:var(--t13); font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:var(--blue);`)}>Настройка под процессы</div>
      <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Что настраиваем во внедрении</h2>
      <p style={s(`font-size:var(--t18); line-height:1.6; color:var(--ink-soft); margin:20px 0 0;`)}>Не «включаем всё подряд», а собираем систему из блоков, которые нужны вашим процессам — без перегруза для команды.</p>
    </div>
    ...
    <div data-reveal-children="120" style={s(`display:grid; grid-template-columns:${v.cols3}; gap:24px; margin-top:56px;`)}>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        ...
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>CRM и воронки продаж</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Стадии под ваш цикл сделки, обязательные поля, карточки сделок и клиентов — без лишних кнопок.</p>
      </div>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        <div style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue);`)}>02</div>
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>Роботы и автоматизация</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Автозадачи, триггеры и напоминания: сделки не зависают, менеджеры не забывают про клиентов.</p>
      </div>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        <div style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue);`)}>03</div>
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>Задачи и проекты</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Канбан, шаблоны задач, чек-листы и сроки — регламенты компании живут в системе, а не в чатах.</p>
      </div>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        <div style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue);`)}>04</div>
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>Телефония и контакт-центр</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Звонки из карточки, записи разговоров, распределение обращений и открытые линии из мессенджеров.</p>
      </div>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        <div style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue);`)}>05</div>
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>Отчёты и дашборды</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Воронка, план-факт, нагрузка менеджеров: руководитель видит картину в системе, а не в Excel по пятницам.</p>
      </div>
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(20px) saturate(180%); -webkit-backdrop-filter:blur(20px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:20px; padding:30px 30px 32px; box-shadow:0 18px 40px -22px rgba(20,23,28,0.2), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        <div style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue);`)}>06</div>
        <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:19px; letter-spacing:-0.01em; margin:14px 0 0; color:var(--ink);`)}>Права и структура</h3>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:10px 0 0;`)}>Отделы, роли и права доступа: каждый видит своё, база клиентов защищена от выгрузки «на память».</p>
      </div>
    </div>
  </section>

  <section id="steps" data-screen-label="Этапы внедрения" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal style={s(`max-width:calc(720px * var(--t-scale));`)}>
      ...
      <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Этапы внедрения Битрикс24</h2>
      <p style={s(`font-size:var(--t18); line-height:1.6; color:var(--ink-soft); margin:20px 0 0;`)}>Объём и сроки фиксируем после аудита — дальше ведём проект по шагам, с понятным результатом на каждом.</p>
    </div>
  ...
  <section id="integrations" data-screen-label="Интеграции" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal style={s(`max-width:calc(760px * var(--t-scale));`)}>
      <div style={s(`font-family:monospace; font-size:var(--t13); font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:var(--violet);`)}>Единый контур</div>
      <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Интеграции: 1С, телефония, сайт и мессенджеры</h2>
      <p style={s(`font-size:var(--t18); line-height:1.6; color:var(--ink-soft); margin:20px 0 0;`)}>CRM работает, когда в ней сходятся все каналы. Подключаем то, чем вы уже пользуетесь.</p>
    </div>

    <div data-reveal-children="120" style={s(`display:grid; grid-template-columns:${v.cols3}; gap:24px; margin-top:56px;`)}>
      <div style={s(`background:var(--paper); border:1px solid var(--line); border-radius:20px; padding:30px;`)}>
        <div style={s(`display:flex; align-items:center; gap:12px;`)}>
          <span style={s(`width:38px; height:38px; border-radius:11px; background:rgba(21,94,239,0.08); border:1px solid rgba(21,94,239,0.18); display:flex; align-items:center; justify-content:center; font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t13); color:var(--blue); flex-shrink:0;`)}>1С</span>
          <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t18); margin:0; color:var(--ink);`)}>1С: Бухгалтерия и УТ</h3>
        </div>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:14px 0 0;`)}>Обмен счетами, оплатами, товарами и остатками. Менеджер видит статус оплаты прямо в сделке — без звонков в бухгалтерию.</p>
      </div>
      <div style={s(`background:var(--paper); border:1px solid var(--line); border-radius:20px; padding:30px;`)}>
        <div style={s(`display:flex; align-items:center; gap:12px;`)}>
          ...
          <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t18); margin:0; color:var(--ink);`)}>Телефония и АТС</h3>
        </div>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:14px 0 0;`)}>Звонки из карточки клиента, записи разговоров в сделке, автосоздание лидов по пропущенным вызовам.</p>
      </div>
      <div style={s(`background:var(--paper); border:1px solid var(--line); border-radius:20px; padding:30px;`)}>
        <div style={s(`display:flex; align-items:center; gap:12px;`)}>
      ...
      <div style={s(`background:var(--paper); border:1px solid var(--line); border-radius:20px; padding:30px;`)}>
        <div style={s(`display:flex; align-items:center; gap:12px;`)}>
          ...
          <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t18); margin:0; color:var(--ink);`)}>Мессенджеры</h3>
        </div>
        <p style={s(`font-size:var(--t15); line-height:1.6; color:var(--ink-soft); margin:14px 0 0;`)}>Telegram, WhatsApp и VK в открытых линиях: вся переписка сохраняется в карточке клиента, отвечать можно из Битрикс24.</p>
      </div>
      <div style={s(`background:var(--paper); border:1px solid var(--line); border-radius:20px; padding:30px;`)}>
        <div style={s(`display:flex; align-items:center; gap:12px;`)}>
  ...
  <section id="migration" data-screen-label="Миграция с других CRM" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal-children="150" style={s(`display:grid; grid-template-columns:${v.migCols}; gap:${v.migGap}; align-items:start;`)}>
      <div>
        <div style={s(`font-family:monospace; font-size:var(--t13); font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:var(--blue);`)}>Переезд в Битрикс24</div>
        <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Миграция в Битрикс24 с amoCRM, Excel и других CRM</h2>
        <p style={s(`font-size:var(--t18); line-height:1.6; color:var(--ink-soft); margin:20px 0 0; max-width:calc(560px * var(--t-scale));`)}>Смена CRM пугает потерей истории. Поэтому переезд у нас поэтапный: сначала перенос на тестовую копию и проверка вместе с вами — и только после этого переключение команды.</p>
        ...
        <div style={s(`display:flex; flex-direction:column; gap:0; margin-top:36px; max-width:calc(560px * var(--t-scale));`)}>
          <div style={s(`display:flex; gap:16px; padding:18px 0; border-top:1px solid var(--line);`)}>
            <span style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue); flex-shrink:0; width:24px;`)}>1.</span>
            <div><span style={s(`font-size:var(--t155); font-weight:600; color:var(--ink);`)}>Аудит и выгрузка данных.</span> <span style={s(`font-size:var(--t155); color:var(--ink-soft);`)}>Смотрим структуру текущей базы, чистим дубли и мусор.</span></div>
          </div>
          <div style={s(`display:flex; gap:16px; padding:18px 0; border-top:1px solid var(--line);`)}>
            <span style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue); flex-shrink:0; width:24px;`)}>2.</span>
            <div><span style={s(`font-size:var(--t155); font-weight:600; color:var(--ink);`)}>Маппинг полей и стадий.</span> <span style={s(`font-size:var(--t155); color:var(--ink-soft);`)}>Сопоставляем воронки, поля и статусы со структурой новой CRM.</span></div>
          </div>
          <div style={s(`display:flex; gap:16px; padding:18px 0; border-top:1px solid var(--line);`)}>
            <span style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue); flex-shrink:0; width:24px;`)}>3.</span>
            <div><span style={s(`font-size:var(--t155); font-weight:600; color:var(--ink);`)}>Тестовый перенос.</span> <span style={s(`font-size:var(--t155); color:var(--ink-soft);`)}>Проверяем на копии: карточки, связи и история на месте.</span></div>
          </div>
          <div style={s(`display:flex; gap:16px; padding:18px 0; border-top:1px solid var(--line); border-bottom:1px solid var(--line);`)}>
            <span style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:var(--t14); color:var(--blue); flex-shrink:0; width:24px;`)}>4.</span>
            <div><span style={s(`font-size:var(--t155); font-weight:600; color:var(--ink);`)}>Боевой перенос.</span> <span style={s(`font-size:var(--t155); color:var(--ink-soft);`)}>Переключаем команду в согласованный день — без остановки продаж.</span></div>
          </div>
        </div>
      </div>
      ...
      <div style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.5)); backdrop-filter:blur(24px) saturate(180%); -webkit-backdrop-filter:blur(24px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:24px; padding:${v.migCardPad}; box-shadow:0 30px 60px -30px rgba(20,23,28,0.25), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        ...
        <div style={s(`display:flex; flex-direction:column; gap:13px; margin-top:20px;`)}>
          ...
          <div style={s(`display:flex; align-items:center; gap:11px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:var(--blue); flex-shrink:0;`)}></span><span style={s(`font-size:var(--t155); color:var(--ink);`)}>История переписки и комментарии</span></div>
          <div style={s(`display:flex; align-items:center; gap:11px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:var(--blue); flex-shrink:0;`)}></span><span style={s(`font-size:var(--t155); color:var(--ink);`)}>Задачи и активности</span></div>
          <div style={s(`display:flex; align-items:center; gap:11px;`)}><span style={s(`width:6px; height:6px; border-radius:50%; background:var(--blue); flex-shrink:0;`)}></span><span style={s(`font-size:var(--t155); color:var(--ink);`)}>Файлы и документы</span></div>
        </div>
        ...
        <div style={s(`display:flex; flex-wrap:wrap; gap:10px; margin-top:18px;`)}>
          ...
          <span style={s(`font-size:var(--t135); font-weight:600; color:var(--ink); background:var(--paper); border:1px solid var(--line); border-radius:999px; padding:8px 16px;`)}>Мегаплан</span>
          <span style={s(`font-size:var(--t135); font-weight:600; color:var(--ink); background:var(--paper); border:1px solid var(--line); border-radius:999px; padding:8px 16px;`)}>RetailCRM</span>
          <span style={s(`font-size:var(--t135); font-weight:600; color:var(--ink); background:var(--paper); border:1px solid var(--line); border-radius:999px; padding:8px 16px;`)}>Самописные CRM</span>
        </div>
        ...
        <div style={s(`display:flex; align-items:center; gap:10px; margin-top:26px; background:rgba(31,138,91,0.07); border:1px solid rgba(31,138,91,0.2); border-radius:12px; padding:14px 16px;`)}>
          ...
          <span style={s(`font-size:var(--t14); font-weight:600; color:#1F8A5B;`)}>Боевой перенос — только после проверки тестового</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section id="sla" data-screen-label="Поддержка и SLA" style={s(`position:relative; background:var(--ink); padding:${v.darkPad}; overflow:hidden;`)}>
    ...
    <div style={s(`position:relative; z-index:1; max-width:var(--wrap); margin:0 auto; padding:${v.darkInnerPad};`)}>
      <div data-reveal style={s(`max-width:680px;`)}>
        <div style={s(`font-family:monospace; font-size:var(--t13); font-weight:700; letter-spacing:0.08em; text-transform:uppercase; background:var(--grad); -webkit-background-clip:text; background-clip:text; color:transparent; display:inline-block;`)}>После запуска</div>
        <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:clamp(32px,4vw,46px); line-height:1.14; letter-spacing:-0.02em; margin:16px 0 0; color:#fff;`)}>Поддержка и сопровождение по SLA</h2>
        <p style={s(`font-size:var(--t18); line-height:1.6; color:rgba(255,255,255,0.65); margin:20px 0 0;`)}>Внедрение — это старт, а не финиш. Берём систему на сопровождение: отвечаем на вопросы, донастраиваем и развиваем её вместе с бизнесом.</p>
      </div>
      ...
      <div data-reveal-children="110" style={s(`display:grid; grid-template-columns:${v.cols3}; gap:20px; margin-top:64px;`)}>
        <div style={s(`display:flex; flex-direction:column; background:linear-gradient(160deg, rgba(255,255,255,0.1), rgba(255,255,255,0.03)); backdrop-filter:blur(20px) saturate(160%); -webkit-backdrop-filter:blur(20px) saturate(160%); border:1px solid rgba(255,255,255,0.16); border-radius:22px; padding:34px 32px; box-shadow:0 24px 60px -20px rgba(0,0,0,0.5), inset 0 1px 1px rgba(255,255,255,0.2);`)}>
          <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:21px; margin:0; color:#fff;`)}>Базовый</h3>
          <p style={s(`font-size:var(--t14); line-height:1.55; color:rgba(255,255,255,0.55); margin:8px 0 0;`)}>Для стабильных систем без частых изменений</p>
          <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--violet); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Консультации и ответы на вопросы команды</span></div>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--violet); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Мелкие правки настроек и прав</span></div>
        ...
        <div style={s(`position:relative; display:flex; flex-direction:column; background:linear-gradient(160deg, rgba(21,94,239,0.22), rgba(18,165,224,0.08)); backdrop-filter:blur(20px) saturate(160%); -webkit-backdrop-filter:blur(20px) saturate(160%); border:1px solid rgba(94,159,255,0.45); border-radius:22px; padding:34px 32px; box-shadow:0 24px 60px -20px rgba(21,94,239,0.35), inset 0 1px 1px rgba(255,255,255,0.25);`)}>
          ...
          <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:#5E9FFF; flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Всё из пакета «Базовый»</span></div>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:#5E9FFF; flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Доработки: новые воронки, роботы, отчёты</span></div>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:#5E9FFF; flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Приоритетная очередь задач</span></div>
        ...
        <div style={s(`display:flex; flex-direction:column; background:linear-gradient(160deg, rgba(255,255,255,0.1), rgba(255,255,255,0.03)); backdrop-filter:blur(20px) saturate(160%); -webkit-backdrop-filter:blur(20px) saturate(160%); border:1px solid rgba(255,255,255,0.16); border-radius:22px; padding:34px 32px; box-shadow:0 24px 60px -20px rgba(0,0,0,0.5), inset 0 1px 1px rgba(255,255,255,0.2);`)}>
          <h3 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:21px; margin:0; color:#fff;`)}>Энтерпрайз</h3>
          <p style={s(`font-size:var(--t14); line-height:1.55; color:rgba(255,255,255,0.55); margin:8px 0 0;`)}>Для компаний, где CRM — критичный процесс</p>
          <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--violet); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Выделенная команда и персональный менеджер</span></div>
            <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--violet); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:rgba(255,255,255,0.85);`)}>Развитие системы: интеграции, смарт-процессы</span></div>
      ...
      <div data-reveal style={s(`display:flex; align-items:center; justify-content:space-between; gap:20px; flex-wrap:wrap; margin-top:36px;`)}>
        <p style={s(`font-size:var(--t15); color:rgba(255,255,255,0.55); margin:0; max-width:calc(560px * var(--t-scale));`)}>Объём часов и условия зависят от вашей системы — подберём формат на консультации.</p>
        <button className="b24fx10" onClick={v.scrollToContact} style={s(`font-family:var(--font-inter),sans-serif; font-size:var(--t14); font-weight:600; color:var(--ink); background:#fff; border:none; padding:13px 28px; border-radius:999px; cursor:pointer; transition:transform 0.2s ease;`)}>Подобрать пакет поддержки</button>
      </div>
    </div>
  </section>

  <section id="packages" data-screen-label="Пакеты внедрения" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal style={s(`max-width:calc(720px * var(--t-scale));`)}>
      <div style={s(`font-family:monospace; font-size:var(--t13); font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:var(--blue);`)}>Форматы работы</div>
      <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Пакеты внедрения</h2>
      <p style={s(`font-size:var(--t18); line-height:1.6; color:var(--ink-soft); margin:20px 0 0;`)}>Три типовых формата — от быстрого запуска до сложных доработок. Точный объём собираем под ваши процессы после аудита.</p>
    </div>
    ...
    <div data-reveal-children="120" style={s(`display:grid; grid-template-columns:${v.cols3}; gap:24px; margin-top:56px; align-items:stretch;`)}>
      <div style={s(`display:flex; flex-direction:column; background:var(--paper); border:1px solid var(--line); border-radius:24px; padding:38px 34px;`)}>
        ...
        <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Одна воронка продаж, базовая настройка</span></div>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Подключение почты и телефонии</span></div>
      ...
      <div style={s(`position:relative; display:flex; flex-direction:column; background:linear-gradient(160deg, rgba(255,255,255,0.85), rgba(255,255,255,0.6)); backdrop-filter:blur(24px) saturate(180%); -webkit-backdrop-filter:blur(24px) saturate(180%); border:1px solid rgba(21,94,239,0.4); border-radius:24px; padding:38px 34px; box-shadow:0 30px 60px -24px rgba(21,94,239,0.3), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
        ...
        <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Несколько воронок, роботы и автоматизация</span></div>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Интеграции: 1С, сайт, телефония, мессенджеры</span></div>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Перенос данных из старой CRM</span></div>
      ...
      <div style={s(`display:flex; flex-direction:column; background:var(--paper); border:1px solid var(--line); border-radius:24px; padding:38px 34px;`)}>
        ...
        <div style={s(`display:flex; flex-direction:column; gap:12px; margin-top:24px;`)}>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Смарт-процессы и нестандартная логика</span></div>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Интеграции по API с внутренними системами</span></div>
          <div style={s(`display:flex; align-items:baseline; gap:10px;`)}><span style={s(`width:5px; height:5px; border-radius:50%; background:var(--blue); flex-shrink:0; transform:translateY(-2px);`)}></span><span style={s(`font-size:var(--t145); color:var(--ink);`)}>Коробочная версия и доработки интерфейса</span></div>
    ...
    <p data-reveal style={s(`font-size:var(--t14); color:var(--ink-faint); margin:26px 0 0;`)}>Стоимость каждого пакета считаем по итогам бесплатного аудита — без «вилок» и скрытых доплат в процессе.</p>
  </section>
  ...
  <section id="faq" data-screen-label="FAQ" style={s(`padding:${v.secPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div style={s(`display:grid; grid-template-columns:${v.faqCols}; gap:${v.migGap}; align-items:start;`)}>
      <div data-reveal>
        ...
        <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:${v.h2Size}; line-height:1.12; letter-spacing:-0.02em; margin:16px 0 0; color:var(--ink);`)}>Частые вопросы о внедрении Битрикс24</h2>
      ...
      <div data-reveal-children="80" style={s(`display:flex; flex-direction:column; gap:14px;`)}>
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg0}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq0} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Сколько стоит внедрение Битрикс24?</h3>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg1}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq1} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Сколько времени занимает внедрение?</h3>
          ...
          <div style={s(`display:grid; grid-template-rows:${v.faqRows1}; transition:grid-template-rows 0.4s cubic-bezier(.16,1,.3,1);`)}>
            <div style={s(`overflow:hidden; min-height:0;`)}><p style={s(`font-size:var(--t15); line-height:1.65; color:var(--ink-soft); margin:0; padding:0 60px 22px 24px;`)}>Базовая настройка CRM — от 2 недель. Внедрение с интеграциями 1С, телефонии и переносом данных — обычно 4–8 недель. Точный срок называем после аудита процессов.</p></div>
          </div>
        </div>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg2}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq2} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Облачный или коробочный Битрикс24 — что выбрать?</h3>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg3}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq3} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Можно ли перенести данные из amoCRM или Excel?</h3>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg4}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq4} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Что будет после запуска?</h3>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg5}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq5} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Битрикс24 уже настроен, но команда им не пользуется. Поможете?</h3>
        ...
        <div style={s(`border:1px solid var(--line); border-radius:16px; background:${v.faqBg6}; overflow:hidden; transition:background 0.3s ease;`)}>
          <div onClick={v.toggleFaq6} style={s(`display:flex; align-items:center; gap:16px; padding:20px 24px; cursor:pointer;`)}>
            <h3 style={s(`font-family:var(--font-inter),sans-serif; font-size:16.5px; font-weight:600; color:var(--ink); margin:0; flex:1; line-height:1.4;`)}>Нужна ли лицензия Битрикс24 и где её купить?</h3>
          ...
          <div style={s(`display:grid; grid-template-rows:${v.faqRows6}; transition:grid-template-rows 0.4s cubic-bezier(.16,1,.3,1);`)}>
            <div style={s(`overflow:hidden; min-height:0;`)}><p style={s(`font-size:var(--t15); line-height:1.65; color:var(--ink-soft); margin:0; padding:0 60px 22px 24px;`)}>Да, для работы нужен тариф Битрикс24. Поможем подобрать редакцию под количество сотрудников и задачи и оформим лицензию — это входит в сопровождение проекта.</p></div>
          </div>
        </div>
      </div>
    </div>
  ...
  <section data-screen-label="Ссылка на ИИ-решения" style={s(`padding:0 ${v.padX}; max-width:var(--wrap); margin:0 auto;`)}>
    <a className="b24fx15" data-reveal href="/ai" style={s(`display:flex; align-items:center; justify-content:space-between; gap:24px; flex-wrap:wrap; background:var(--ink); border-radius:24px; padding:${v.bannerPad}; text-decoration:none; position:relative; overflow:hidden; transition:transform 0.25s ease;`)}>
      ...
      <span style={s(`position:relative; z-index:1; display:inline-flex; align-items:center; gap:8px; font-size:var(--t15); font-weight:600; color:#fff; border:1px solid rgba(255,255,255,0.3); border-radius:999px; padding:13px 26px; white-space:nowrap;`)}>Подробнее об ИИ →</span>
    </a>
  </section>
  ...
  <section id="contact" data-screen-label="Контакты CTA" style={s(`padding:${v.contactSecPad}; max-width:var(--wrap); margin:0 auto;`)}>
    <div data-reveal style={s(`background:linear-gradient(160deg, rgba(255,255,255,0.72), rgba(255,255,255,0.48)); backdrop-filter:blur(28px) saturate(180%); -webkit-backdrop-filter:blur(28px) saturate(180%); border:1px solid rgba(255,255,255,0.75); border-radius:28px; padding:${v.contactPadding}; display:grid; grid-template-columns:${v.contactColumns}; gap:56px; position:relative; overflow:hidden; box-shadow:0 50px 90px -50px rgba(20,23,28,0.28), inset 0 1px 1px rgba(255,255,255,0.9);`)}>
      ...
      <div style={s(`position:relative; z-index:1;`)}>
        <h2 style={s(`font-family:var(--font-manrope),sans-serif; font-weight:800; font-size:clamp(30px,3.6vw,42px); letter-spacing:-0.02em; line-height:1.15; color:var(--ink); margin:0;`)}>Обсудим внедрение Битрикс24?</h2>
        <p style={s(`font-size:var(--t17); line-height:1.6; color:var(--ink-soft); margin:18px 0 0; max-width:calc(420px * var(--t-scale));`)}>Расскажите, как устроены продажи сейчас, — предложим план внедрения, сроки и оценку. Консультация бесплатная.</p>
  ...
  <footer style={s(`padding:0 ${v.padX} 48px; max-width:var(--wrap); margin:0 auto; display:flex; flex-direction:${v.footerDirection}; align-items:${v.footerAlign}; justify-content:space-between; gap:24px; border-top:1px solid var(--line); padding-top:36px;`)}>
    ...
    <div style={s(`display:flex; gap:24px; flex-wrap:wrap;`)}>
      ...
      <a href="#steps" style={s(`font-size:var(--t14); color:var(--ink-soft); text-decoration:none;`)}>Этапы</a>
      <a href="#sla" style={s(`font-size:var(--t14); color:var(--ink-soft); text-decoration:none;`)}>Поддержка</a>
