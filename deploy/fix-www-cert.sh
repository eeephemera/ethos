...
Значит, доступность извне всё ещё ограничена. Выпустите через DNS — этот способ
не требует, чтобы порт 80 был доступен из интернета:

  certbot certonly --manual --preferred-challenges dns --expand \\
    --agree-tos -m ${EMAIL:-<ваш-email>} -d $DOMAIN -d $WWW

Certbot попросит ДВЕ TXT-записи с одинаковым именем _acme-challenge —
добавьте обе (не удаляя первую), дождитесь распространения:
